...
<div class="carpentries-callout" markdown="1">
**Tip: Why does `?read.csv` open the documentation for `read.table`?**
...
and `read.delim` to read in tables separated by `\t` (tabs). If you
know how your table is separated, you can use one of the provided
short cuts, but in case you run into an unconventional separator you
are now equipped with the knowledge to define it in the `sep = `
argument of `read.table`!

</div>

Now, let's read in the file `combined_tidy_vcf.csv` that we copied into `~/day2` (where we should be). Call this data `variants`. The
