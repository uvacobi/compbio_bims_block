...
Let's use `ls -F` to list the new directories we just created in the `project` directory:
...
It is good practice to use all lowercase letters in names of files and directories.
Linux is **case-sensitive**, so `thesis` and `Thesis` are treated as two different directories.
This differs from Windows and macOS, which are typically case-insensitive.
...
after this workshop. Many programmers use [Emacs](https://www.gnu.org/software/emacs/) or
[Vim](https://www.vim.org/) (both of which require more time to learn),
or [VSCode](https://code.visualstudio.com/) connected to the HPC via Remote SSH.
...
![screenshot of nano text editor in action with the text It's not publish or perish any more, it's share and thrive]({{ "/day1/fig/nano-screenshot.png" | relative_url }})
...
1. The `touch` command generates a new file called `my_file.txt` in
  your current directory.  You can observe this newly generated file by typing `ls` at the
  command line prompt.
...
When using two arguments with `mv` or `cp`,
the second argument can be the location and new name we want the file to have.
Alternatively, if we provide the path to an existing directory as the second argument,
`mv`/`cp` will interpret that to mean we want the file moved or copied
into that directory without changing its name.
...
`cp` copies the other files specified into that directory.
If the last argument is a file name, you get an error:
