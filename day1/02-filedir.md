...
<div class="carpentries-instructor" markdown="1">
**Instructor**


Introducing and navigating the filesystem in the shell
(covered in [Navigating Files and Directories](02-filedir.md) section)
can be confusing. You may open the Open OnDemand file browser alongside the terminal
so learners can see the directory structure while navigating with shell commands.
...
On UVA HPC, your home directory path will be `/home/<computing_id>`, for example `/home/mst3k`. All examples in this lesson use `/home/nelle` to represent the home directory — substitute your own computing ID when running commands on UVA HPC.

We will assume that your `pwd` command returns your home directory. If it returns something different, navigate there with `cd ~` before continuing.
...
The diagram below shows a typical Linux filesystem layout — similar to what you see on UVA HPC.

![The file system is made up of a root directory that contains sub-directories titled bin, data, home, and tmp]({{ "/day1/fig/filesystem.svg" | relative_url }})
...
`scratch` (for temporary large files during computations),
`tmp` (for temporary files that don't need to be stored long-term),
`usr` (which contains user programs and libraries),
and so on.
...
Underneath `/home`,
we find one directory for each user with an account on the system,
such as *imhotep*, *larry*, and *nelle*.

![Like other directories, home directories are sub-directories underneath "/home" like "/home/imhotep", "/home/larry" or"/home/nelle"]({{ "/day1/fig/home-directories.svg" | relative_url }})
...
(Your output may include additional directories depending on what you have created previously in your account.)
...
will not be able to scroll back; using `clear -x` prevents this.
An alternative is the keyboard shortcut <kbd>Ctrl</kbd>+<kbd>L</kbd>.
...
1. We can pass a `--help` option to any command, for example:
...
Both work on UVA HPC. We'll describe both ways next.
...
<div class="carpentries-challenge" markdown="1">
**Challenge**
...
in `/home/nelle/shell-lesson-data/exercise-data`.

If we run `ls -F` without arguments now,
it lists the contents of `/home/nelle/shell-lesson-data/exercise-data`,
...
```output
/home/nelle/shell-lesson-data/exercise-data
```
...
Sure enough,
if we run `pwd` after running `cd ..`, we're back in `/home/nelle/shell-lesson-data`:
...
```bash
$ cd /home/nelle/shell-lesson-data
```
...
3. Yes: absolute path to Nelle's home directory.
...
![A directory tree below the Users directory where "/home" contains the directories "backup" and "thing"; "/home/backup" contains "original","pnas\_final" and "pnas\_sub"; "/home/thing" contains "backup"; and"/home/thing/backup" contains "2012-12-01", "2013-01-08" and"2013-01-27"]({{ "/day1/fig/filesystem-challenge.svg" | relative_url }})
...
![General syntax of a shell command]({{ "/day1/fig/shell_command_syntax.svg" | relative_url }})
...
```output
total 1
0 alkanes  0 animal-counts  0 creatures  1 numbers.txt  0 writing
```
...
On UVA HPC's parallel filesystem, directories often report 0 blocks.
...
```output
numbers.txt  alkanes  creatures  writing  animal-counts
```
...
```output
apps@   etc/    local/         mnt/     project@  scratch@  standard@  var/
bin@    home@   localscratch/  nv/      root/     sfs/      sys/
boot/   lib@    lost+found/    opt/     run/      share/    tmp/
dev/    lib64@  media/         proc/    sbin@     srv/      usr/
```
