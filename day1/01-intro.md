...
A **command-line interface** (CLI) allows users to interact with a computer by reading
...
In this workshop, we will be using Bash, the most popular Unix shell,
and the default shell on UVA HPC.
...
In fact, tools and resources such as high-performance computing systems like UVA HPC
require users to be familiar with the shell — it is the primary way to submit jobs,
manage files, and run software on the cluster.

Using the shell will take some effort and some time to learn.
While a GUI presents you with choices to select, CLI choices are not automatically presented to you.
It can be daunting at first, but once you have become familiar with this style
...
On UVA HPC your prompt will look like this:

```bash
[mst3k@udc-ba37-36c1 ~]$
```

where `mst3k` is your computing ID, `udc-ba37-36c1` is the login node, and `~` is shorthand for your home directory. Do not worry about these details — the only important character to focus on is the `$` at the end, which signals that the shell is ready for your input. The lesson examples use a plain `$` for brevity.
